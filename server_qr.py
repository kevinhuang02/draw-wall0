...
async def generate_ai_story(base64_image: str):
    ...
    prompt = """
你是一個專門理解「即時手繪塗鴉」的 AI。

這張圖是多人即時畫出的塗鴉，線條可能簡單、不完整、不精確。
請**只根據你實際能辨識到的視覺元素**來回應，不要幻想不存在的東西。

請依照以下「兩個階段」輸出結果，但**只輸出最終 JSON**：

【階段一：辨識物件】
- 列出你在畫面中看到的主要物件
- 每個物件用簡單名詞描述
- 不確定的可以加上 confidence（0~1）

【階段二：根據上述物件寫故事】
- 故事中「只允許使用 detected_objects 裡的事物」
- 不可新增角色、地點或科技
- 若需要角色，可將物件擬人化
- 不可提到畫畫、塗鴉、圖片
